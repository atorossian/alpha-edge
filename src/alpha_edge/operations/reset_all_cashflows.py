...
def list_cashflow_keys(s3, *, bucket: str, prefix: str) -> list[str]:
    keys: list[str] = []
    token = None
    while True:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Back up and delete every cashflow JSON object in the active engine root.")
    ...
    keys = list_cashflow_keys(s3, bucket=cfg.bucket, prefix=prefix)

    ts = dt.datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")
    tag = args.backup_tag or f"full_reset_{ts}"
    backup_root = f"{cfg.engine_root.strip('/')}/cashflows_audit/{tag}"

    print("=== CASHFLOW FULL RESET ===")
    print("env:", cfg.env)
    print("bucket:", cfg.bucket)
    print("region:", cfg.region)
    print("engine_root:", cfg.engine_root)
    print("prefix:", prefix)
    print("cashflow_objects_found:", len(keys))
